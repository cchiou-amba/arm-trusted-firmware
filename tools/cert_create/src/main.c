...
static const cmd_opt_t common_cmd_opt[] = {
	...
	},
	{
		...
		"'ecdsa', 'ecdsa-brainpool-regular', 'ecdsa-brainpool-twisted', 'ed25519'"
#else
		"Key algorithm: 'rsa' (default)- RSAPSS scheme as per PKCS#1 v2.1, " \
...
int main(int argc, char *argv[])
{
	...
	/* Load default provider first to ensure DRBG is initialized */
	/* This is required for s2n library used by AWS SDK */

	provider = OSSL_PROVIDER_load(libctx, AWS_KMS_PROVIDER_PATH_STR);
	if (provider == NULL) {
		ERROR("Cannot load aws-kms provider\n");
	...
	if (default_provider == NULL) {
		ERROR("Cannot load default OpenSSL provider\n");
		exit(1);
	}



	/* Get the provider name and set it for key_load() to use */
	/* This ensures we use AWS KMS provider instead of default provider */
	{
		const char *provider_name = OSSL_PROVIDER_get0_name(provider);
		if (provider_name != NULL) {
			key_set_aws_kms_provider_name(provider_name);
		}
	}
#else
